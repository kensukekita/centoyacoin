...
#if defined(NDEBUG)
# error "Centoyacoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x1a902482bafc35d9d53a802c5cdc40584aa98304f64bd1a28e1219ea8de8edb5");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Centoyacoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Centoyacoin Signed Message:\n";
...
{
    // Centoyacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Centoyacoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Centoyacoin: 3.5 days
static const int64 nTargetSpacing = 1 * 30; // Centoyacoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Centoyacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Centoyacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x1a902482bafc35d9d53a802c5cdc40584aa98304f64bd1a28e1219ea8de8edb5");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 05/Oct/2011 Steve centoyacoin xd";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Centoyacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CentoyacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CentoyacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CentoyacoinMiner(CWallet *pwallet)
{
    printf("CentoyacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("centoyacoin-miner");
    ...
    try { loop {
        ...
        printf("Running CentoyacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CentoyacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CentoyacoinMiner, pwallet));
}
